...
/// Encoded AVC420 (H.264 4:2:0) data for a surface region
///
/// Delivered to [`GraphicsPipelineHandler::on_encoded_avc420`] before optional
/// software decode. `data` is in RDP AVC format: H.264 NAL units prefixed with
/// 4-byte big-endian lengths.
#[derive(Debug, Clone)]
#[non_exhaustive]
pub struct EncodedAvc420Frame {
    /// Surface this update applies to
    pub surface_id: u16,
    /// Destination rectangle within the surface (exclusive `right`/`bottom`)
    pub destination_rectangle: ExclusiveRectangle,
    /// Logical EGFX frame ID when inside a StartFrame/EndFrame pair
    pub frame_id: Option<u32>,
    /// Logical EGFX frame timestamp in milliseconds when inside a StartFrame/EndFrame pair
    pub timestamp_ms: Option<u64>,
    /// Dirty rectangles carried by the AVC420 bitmap stream
    pub rectangles: Vec<InclusiveRectangle>,
    /// Per-rectangle quantization/quality metadata
    pub quant_qual_vals: Vec<crate::pdu::QuantQuality>,
    /// Length-prefixed H.264 NAL units from the AVC420 bitmap stream
    pub data: Vec<u8>,
}
...
pub trait GraphicsPipelineHandler: Send {
    ...
    /// advertisement time unless an H.264 decoder is configured on the
    /// [`GraphicsPipelineClient`] or [`GraphicsPipelineHandler::wants_encoded_avc420`]
    /// returns `true`. If all returned sets require AVC and the client cannot
    /// consume AVC, a V8-only fallback is used.
    ...
    /// Returns whether this handler wants encoded AVC420 frames.
    ///
    /// Set this to `true` when the handler consumes H.264 directly instead of
    /// requiring the EGFX client to decode to RGBA.
    fn wants_encoded_avc420(&self) -> bool {
        false
    }

    /// Called when encoded AVC420 data is available for a surface.
    ///
    /// This callback is invoked after the AVC420 bitmap stream is parsed and
    /// before optional software decode. It is also invoked when no H.264 decoder
    /// is configured, provided [`GraphicsPipelineHandler::wants_encoded_avc420`]
    /// returns `true`.
    fn on_encoded_avc420(&mut self, _frame: &EncodedAvc420Frame) {}
...
pub struct GraphicsPipelineClient {
    ...
    current_frame_timestamp_ms: Option<u64>,
    frames_queued: u32,
...
impl GraphicsPipelineClient {
    ...
    pub fn new(handler: Box<dyn GraphicsPipelineHandler>, h264_decoder: Option<Box<dyn H264Decoder>>) -> Self {
        Self {
            ...
            current_frame_timestamp_ms: None,
            frames_queued: 0,
    ...
    pub fn handle_pdu_for_tests(&mut self, pdu: GfxPdu) -> PduResult<Vec<DvcMessage>> {
        self.handle_pdu(pdu)
    }
    ...
    fn handle_pdu(&mut self, pdu: GfxPdu) -> PduResult<Vec<DvcMessage>> {
        match pdu {
            ...
            GfxPdu::StartFrame(start) => {
                ...
                self.current_frame_timestamp_ms = Some(start.timestamp.as_millis());
    ...
    fn handle_reset_graphics(&mut self, width: u32, height: u32) {
        ...
        self.current_frame_timestamp_ms = None;
    ...
    fn decode_avc420(&mut self, surface_id: u16, dest_rect: &ExclusiveRectangle, bitmap_data: &[u8]) -> PduResult<()> {
        ...
        if self.handler.wants_encoded_avc420() {
            let frame = EncodedAvc420Frame {
                surface_id,
                destination_rectangle: dest_rect.clone(),
                frame_id: self.current_frame_id,
                timestamp_ms: self.current_frame_timestamp_ms,
                rectangles: stream.rectangles.clone(),
                quant_qual_vals: stream.quant_qual_vals.clone(),
                data: stream.data.to_vec(),
            };
            self.handler.on_encoded_avc420(&frame);
        }
    ...
    fn decode_clearcodec(&mut self, pdu: crate::pdu::WireToSurface1Pdu) {
        let dest_width = pdu.destination_rectangle.width();
        let dest_height = pdu.destination_rectangle.height();
        let bgra_data = match self
            .clearcodec_decoder
            .decode(&pdu.bitmap_data, dest_width, dest_height)
        {
            Ok(data) => data,
            Err(error) => {
                warn!(
                    ...
                    error = %error,
                    "ClearCodec decode failed; skipping bitmap tile"
                );
                return;
            }
        };
...
impl DvcProcessor for GraphicsPipelineClient {
    ...
    fn start(&mut self, _channel_id: u32) -> PduResult<Vec<DvcMessage>> {
        let can_consume_avc420 = self.h264_decoder.is_some() || self.handler.wants_encoded_avc420();
        let caps = if can_consume_avc420 {
...
mod tests {
    use super::*;
    use crate::pdu::{WireToSurface1Pdu, StartFramePdu, Timestamp};
    ...
    #[derive(Default)]
    struct TestHandler {
        encoded_avc420: Arc<Mutex<Vec<EncodedAvc420Frame>>>,
        bitmap_updates: Arc<Mutex<Vec<BitmapUpdateSnapshot>>>,
    }

    impl TestHandler {
        fn with_encoded_store(encoded_avc420: Arc<Mutex<Vec<EncodedAvc420Frame>>>) -> Self {
            Self {
                encoded_avc420,
                bitmap_updates: Arc::new(Mutex::new(Vec::new())),
            }
        }

        fn with_bitmap_store(bitmap_updates: Arc<Mutex<Vec<BitmapUpdateSnapshot>>>) -> Self {
            Self {
                encoded_avc420: Arc::new(Mutex::new(Vec::new())),
                bitmap_updates,
            }
        }
    }
    ...
    impl GraphicsPipelineHandler for TestHandler {
        ...
        fn on_bitmap_updated(&mut self, update: &BitmapUpdate) {
            self.bitmap_updates
                .lock()
                .expect("bitmap store poisoned")
                .push(BitmapUpdateSnapshot {
        ...
        fn on_encoded_avc420(&mut self, frame: &EncodedAvc420Frame) {
            self.encoded_avc420.lock().expect("encoded store poisoned").push(frame.clone());
        }
        fn on_frame_complete(&mut self, _frame_id: u32) {}
        fn on_close(&mut self) {}
        fn on_unhandled_pdu(&mut self, _pdu: &GfxPdu) {}
    ...
    fn state_transitions() {
        let mut client = GraphicsPipelineClient::new(Box::new(TestHandler::default()), None);
    ...
    fn reset_graphics_clears_surfaces_and_frame_tracking() {
        ...
        let _ = client.handle_pdu(GfxPdu::StartFrame(StartFramePdu {
            timestamp: Timestamp {
                milliseconds: 0,
                seconds: 0,
                minutes: 0,
    ...
    #[test]
    fn avc420_payload_is_delivered_before_decode_even_without_decoder() {
        let encoded_store = Arc::new(Mutex::new(Vec::new()));
        let handler = TestHandler::with_encoded_store(encoded_store.clone());
        ...
        client
            ...
            .unwrap();
        client
            .handle_pdu(GfxPdu::StartFrame(StartFramePdu {
                timestamp: Timestamp {
                    milliseconds: 34,
                    seconds: 12,
                    minutes: 0,
        ...
        let h264_payload = vec![0, 0, 0, 3, 0x67, 0x42, 0x00];
        ...
        let mut bitmap_data = vec![0; avc_stream.size()];
        avc_stream
            .encode(&mut WriteCursor::new(&mut bitmap_data))
            .unwrap();

        client
            .handle_pdu(GfxPdu::WireToSurface1(WireToSurface1Pdu {
                surface_id: 7,
                codec_id: Codec1Type::Avc420,
                pixel_format: PixelFormat::XRgb,
        ...
        let frames = encoded_store.lock().expect("encoded store poisoned");
        assert_eq!(frames.len(), 1);
        let frame = &frames[0];
        assert_eq!(frame.surface_id, 7);
        assert_eq!(frame.frame_id, Some(42));
        assert_eq!(frame.timestamp_ms, Some(12_034));
        assert_eq!(frame.destination_rectangle.left, 0);
        assert_eq!(frame.destination_rectangle.right, 640);
        assert_eq!(frame.data, h264_payload);
        assert_eq!(frame.rectangles.len(), 1);
        assert_eq!(frame.quant_qual_vals.len(), 1);
    }

    #[test]
    fn clearcodec_wire_to_surface_is_decoded_to_bitmap_update() {
        let bitmap_store = Arc::new(Mutex::new(Vec::new()));
        let handler = TestHandler::with_bitmap_store(bitmap_store.clone());
        let mut client = GraphicsPipelineClient::new(Box::new(handler), None);
        ...
        let bgra = vec![
            0x00, 0x80, 0xFF, 0xFF, // red-ish pixel in BGRA
            0x10, 0x20, 0x30, 0xFF, // dark pixel in BGRA
        ];
        let bitmap_data = encoder.encode(&bgra, 2, 1);
        ...
        let updates = bitmap_store.lock().expect("bitmap store poisoned");
        assert_eq!(updates.len(), 1);
        assert_eq!(
            updates[0],
            BitmapUpdateSnapshot {
                surface_id: 3,
                codec_id: Codec1Type::ClearCodec,
                width: 2,
                height: 1,
                data: vec![0xFF, 0x80, 0x00, 0xFF, 0x30, 0x20, 0x10, 0xFF],
            }
        );
    }

    #[test]
    fn invalid_clearcodec_wire_to_surface_does_not_abort_frame() {
        let bitmap_store = Arc::new(Mutex::new(Vec::new()));
        let handler = TestHandler::with_bitmap_store(bitmap_store.clone());
        let mut client = GraphicsPipelineClient::new(Box::new(handler), None);
        ...
        let wire_result = client.handle_pdu(GfxPdu::WireToSurface1(WireToSurface1Pdu {
            surface_id: 9,
            codec_id: Codec1Type::ClearCodec,
            pixel_format: PixelFormat::XRgb,
            destination_rectangle: ExclusiveRectangle {
            ...
            bitmap_data: vec![0xFF, 0x00, 0x10],
        }));

        assert!(wire_result.is_ok());
        assert!(bitmap_store.lock().expect("bitmap store poisoned").is_empty());
    ...
    #[test]
    fn start_advertises_avc_capabilities_when_encoded_avc420_is_requested_without_decoder() {
        struct EncodedOnlyHandler;

        impl GraphicsPipelineHandler for EncodedOnlyHandler {
            fn wants_encoded_avc420(&self) -> bool {
                true
            }
        }

        let mut client = GraphicsPipelineClient::new(Box::new(EncodedOnlyHandler), None);

        let messages = client.start(1).unwrap();
        ...
        assert!(
            ...
            "encoded AVC420 consumers need AVC-capable capability sets"
        );
    }
}
