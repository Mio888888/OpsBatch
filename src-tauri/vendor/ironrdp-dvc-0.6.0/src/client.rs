...
pub struct DrdynvcClient {
    ...
    diagnostics_label: Option<String>,
    diagnostics_data_pdus_seen: u64,
}
...
impl DrdynvcClient {
    ...
    pub fn new() -> Self {
        Self {
            ...
            diagnostics_data_pdus_seen: 0,
        }
    }

    #[must_use]
    pub fn with_diagnostics_label(mut self, label: impl Into<String>) -> Self {
        self.diagnostics_label = Some(label.into());
        self
    }

    pub fn diagnostics_label(&self) -> Option<&str> {
        self.diagnostics_label.as_deref()
    }
    ...
    fn create_capabilities_response(&mut self, server_version: CapsVersion) -> SvcMessage {
        ...
        self.log_diagnostics(format_args!(
            "drdynvc_capabilities_response version={server_version:?}"
        ));
    ...
    pub fn close_channel(&mut self, channel_id: u32) -> Option<SvcMessage> {
        ...
        self.log_diagnostics(format_args!("drdynvc_close_send channel_id={channel_id}"));
        Some(SvcMessage::from(DrdynvcClientPdu::Close(ClosePdu::new(channel_id))))
    }

    fn should_log_diagnostics_count(count: u64) -> bool {
        count <= 5 || count % 60 == 0
    }

    fn data_pdu_stats(data: &DrdynvcDataPdu) -> (&'static str, usize, Option<u32>) {
        match data {
            DrdynvcDataPdu::DataFirst(pdu) => ("first", pdu.data().len(), Some(pdu.length())),
            DrdynvcDataPdu::Data(pdu) => ("data", pdu.data().len(), None),
        }
    }

    fn creation_status_name(status: CreationStatus) -> &'static str {
        if status == CreationStatus::OK {
            "OK"
        } else if status == CreationStatus::NO_LISTENER {
            "NO_LISTENER"
        } else if status == CreationStatus::NOT_FOUND {
            "NOT_FOUND"
        } else {
            "UNKNOWN"
        }
    ...
    #[cfg(any(feature = "std", feature = "opsbatch-diagnostics"))]
    fn log_diagnostics(&self, args: fmt::Arguments<'_>) {
        if let Some(label) = self.diagnostics_label.as_deref() {
            std::eprintln!("[RDP][backend][{}] {}", label, args);
        }
    }

    #[cfg(not(any(feature = "std", feature = "opsbatch-diagnostics")))]
    fn log_diagnostics(&self, _args: fmt::Arguments<'_>) {}
}
    ...
    fn process(&mut self, payload: &[u8]) -> PduResult<Vec<SvcMessage>> {
        ...
        match pdu {
            DrdynvcServerPdu::Capabilities(caps_request) => {
                ...
                let version = caps_request.version();
                self.log_diagnostics(format_args!(
                    "drdynvc_capabilities_request version={version:?}"
                ));
                responses.push(self.create_capabilities_response(version));
            }
            DrdynvcServerPdu::Create(create_request) => {
                ...
                let listener_available = self.dynamic_channels.has_listener(&channel_name);
                self.log_diagnostics(format_args!(
                    "drdynvc_create_request channel_id={} name={} listener={}",
                    channel_id, channel_name, listener_available,
                ));
                ...
                if !self.cap_handshake_done {
                    ...
                    self.log_diagnostics(format_args!(
                        "drdynvc_create_before_capabilities channel_id={} name={}",
                ...
                self.log_diagnostics(format_args!(
                    "drdynvc_create_response channel_id={} name={} status={} start_messages={}",
                    channel_id,
                    channel_name,
                    Self::creation_status_name(creation_status),
                    start_messages.len(),
                ));
            ...
            DrdynvcServerPdu::Close(close) => {
                ...
                if self.dynamic_channels.remove_by_channel_id(channel_id).is_some() {
                    ...
                    self.log_diagnostics(format_args!(
                        "drdynvc_close_received channel_id={} response=true",
                        channel_id,
                    ));
                    responses.push(SvcMessage::from(close_response));
                } else {
                    self.log_diagnostics(format_args!(
                        "drdynvc_close_received channel_id={} response=false",
                        channel_id,
                    ));
                }
            }
            DrdynvcServerPdu::Data(data) => {
                ...
                self.diagnostics_data_pdus_seen = self.diagnostics_data_pdus_seen.saturating_add(1);
                if Self::should_log_diagnostics_count(self.diagnostics_data_pdus_seen) {
                    let (kind, chunk_bytes, total_bytes) = Self::data_pdu_stats(&data);
                    let total_bytes = total_bytes
                        .map(|value| value.to_string())
                        .unwrap_or_else(|| "-".to_owned());
                    self.log_diagnostics(format_args!(
                        "drdynvc_data count={} channel_id={} name={} kind={} chunk_bytes={} total_bytes={}",
                        self.diagnostics_data_pdus_seen,
                        channel_id,
...
impl DynamicChannelSet {
    ...
    fn has_listener(&self, name: &DynamicChannelName) -> bool {
