...
OpsBatch 是一个面向运维场景的本地桌面工作台，用于集中管理服务器资产、SSH 终端、RDP/VNC 远程桌面、批量命令、脚本库、文件传输、自动化工作流与运维知识同步。桌面端基于 Tauri v2、React 19、TypeScript/Vite 与 Rust 后端构建，核心数据持久化在本机 SQLite 数据库中。
...
- 将 SSH、SFTP、RDP、VNC、端口转发和本地终端放进同一套主机上下文。
...
- RDP 连接：Windows 主机可通过独立 RDP 窗口连接，主机配置包含域、桌面分辨率、剪贴板、音频和磁盘映射选项；前端支持 H.264 直显/bitmap 两种呈现路径。
- VNC 连接：VNC 主机可通过 noVNC 客户端和本地 WebSocket bridge 连接，主机配置包含端口、用户名、密码、共享连接和只读模式。
- 分辨率边界：VNC 默认请求 `1280x720`，显示尺寸限制在 `1920x1080` 内；RDP 桌面配置限制在 `640x480` 到 `3840x2160` 之间。
- 调试与日志：RDP/VNC 页面会输出连接、帧率、传输和诊断日志，便于排查连接失败、认证失败和刷新性能问题。

### 批量执行与文件分发

- 批量命令执行：对选中主机执行命令，支持并发数、超时、实时结果、失败主机重试和历史记录；连接信号量限制单次并发为 4，避免瞬间打满连接。
- 危险命令防护：内置和自定义危险命令规则，在执行高风险命令前触发确认；也可结合 AI 做风险说明；正则规则已预编译，消除终端按键时的重复编译开销。
...
| 拖拽/动效 | dnd-kit、GSAP |
| 后端语言 | Rust |
| 后端能力 | Tauri commands、SQLite (`rusqlite`)、SSH/SFTP (`russh`, `russh-sftp`)、RDP (`ironrdp`)、VNC WebSocket bridge、WebRTC/H.264 远程桌面路径、本地 PTY、Git (`git2`)、HTTP (`reqwest`) |
| 本地数据 | Tauri app data 目录下的 SQLite 数据库 `opsbatch.db`（r2d2 连接池） |
...
│   ├── src/commands/         # hosts、terminal、execution、sftp、rdp、vnc、workflow、ai 等命令模块
...
- 主机密码、私钥字段、Git 仓库 Token 和 AI API Key 会写入本地加密 vault，SQLite 中保存 `***keychain***` 占位符；旧版本已经写入系统 keychain 的业务凭据会在首次读取时迁移到本地加密 vault。
- 本地加密 vault 的主密钥存放在系统钥匙串 / 凭据管理器中。每次启动会先解锁一次并把主密钥缓存在本进程内，后续连接、广播终端、同步仓库或调用 AI 时不再重复请求系统钥匙串授权。
...
OpsBatch 面向授权运维场景，提供 SSH、SFTP、RDP/VNC 远程桌面、批量命令、文件分发、工作流和 AI 辅助等能力。工具本身不会替用户取得授权，也不会替代组织内部审批、审计、变更管理和安全制度。

### 网络合规与授权访问

- 仅应在自己拥有、管理或已获得明确授权的服务器、账号、网络、云资源和数据范围内使用 OpsBatch。
- 批量命令、脚本、文件分发、端口转发、远程桌面操作和工作流可能造成配置变更、数据移动、权限变化或服务中断；执行前应确认目标范围、命令影响、审批要求和回滚方案。
...
- 主机密码、私钥字段、Git 仓库 Token 和 AI API Key 会写入本地加密 vault，启动解锁后本次运行会复用内存中的 vault 主密钥；云厂商凭据、历史数据或部分运行时配置仍可能保存在本地数据库或配置文件中。
...
- 数据库连接池：替换单连接 Mutex 为 r2d2 连接池，优化高并发场景下的锁竞争。
- SSH 连接池：消除每连接独占 tokio Runtime，统一走连接池共享 Runtime，降低批量并发资源占用。
- 批量执行内存：用 `Arc<str>` 替代 String 反复 clone，减少批量执行中的内存分配。
- 正则预编译：危险命令正则和 RAG tokenize/chunk 正则提升为全局静态常量，避免运行时重复编译。
- 文件预览内存：消除 SFTP 文件预览中的 `Array<number>` 中间表示，降低约 5 倍内存膨胀。
- 首屏加载：延迟加载 CodeEditor 组件，优化 prefetchPages 分批预加载，修复懒加载失效问题。
- SSH 闲置回收：修复 idle reaper 静默断连不回收问题，增加主动健康检查。
- 主机监控：移除 SSH 监控快照中的 sleep 等待，改为前端累积值差分计算 CPU 使用率。
...
- 远程桌面：RDP/VNC 已接入独立窗口和后端会话管理；VNC 仍受目标服务端编码、网络质量、noVNC 渲染和被控端帧更新策略影响，不应承诺达到原生客户端同等流畅度。
