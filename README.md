...
- 主机密码、私钥字段、Git 仓库 Token 和 AI API Key 会写入本地加密 vault，SQLite 中保存 `***keychain***` 占位符；旧版本已经写入系统 keychain 的凭据会在首次读取时迁移到本地加密 vault。
- 本地加密 vault 使用独立主密钥文件加密保存，避免系统钥匙串授权弹窗；安全强度仍依赖设备登录保护、文件权限、磁盘加密和备份位置。
...
- 主机密码、私钥字段、Git 仓库 Token 和 AI API Key 会写入本地加密 vault，但云厂商凭据、历史数据或部分运行时配置仍可能保存在本地数据库或配置文件中。
